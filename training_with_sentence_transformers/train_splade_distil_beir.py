...
model_save_path = f'{args.output_dir}/distilSplade_{args.lambda_q}_{args.lambda_d}_{args.model_name.replace("/", "-")}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
train_dataset = MSMARCODataset(train_queries, corpus=corpus, ce_scores=ce_scores)
